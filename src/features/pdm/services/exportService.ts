...
export interface ExportServiceInterface {
  ...
  readonly generateFileName: (format: ExportFormat, baseTitle?: string) => string;
}
...
export class MockExportService implements ExportServiceInterface {
  ...
  private generateCSVContent(data: ExportData): string {
    ...
    const rows = data.selectedMatches.map((match: EquivalenceMatch) => this.generateCSVRow(match, data));
  ...
  private generatePDFContent(data: ExportData): string {
    ...
    return `
      ...
      ${data.selectedMatches.map((match: EquivalenceMatch, index: number) => `
        ${index + 1}. ${match.nome}
        ...
        ${data.includeSpecs && match.especificacoesTecnicas ? 'Specifications: ' + JSON.stringify(match.especificacoesTecnicas, null, 2) : ''}
      `).join('\n')}
    `;
  }
